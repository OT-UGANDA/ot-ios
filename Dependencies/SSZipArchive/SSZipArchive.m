...
+ (BOOL)isEncrypted:(NSString *)path {
    zipFile unzFile = unzOpen((const char*)[path UTF8String]);
    ...
    if (ret == UNZ_OK) {
        do {
            ...
            ret = unzGetCurrentFileInfo(unzFile, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if (ret!= UNZ_OK) {
                return NO;
            }
...
+ (BOOL)createZipFileAtPath:(NSString *)path withFilesAtPaths:(NSArray *)paths password:(NSString *)password {
	...
	if ([zipArchive open]) {
		for (NSString *path in paths) {
			[zipArchive writeFile:path password:password];
		}
...
{
    return [self writeFileAtPath:path withFileName:nil password:password];
}

// supports writing files with logical folder/directory structure
// *path* is the absolute path of the file that will be compressed
// *fileName* is the relative name of the file how it is stored within the zip e.g. /folder/subfolder/text1.txt
- (BOOL)writeFileAtPath:(NSString *)path withFileName:(NSString *)fileName password:(NSString *)password {
    ...
    if ([password length] == 0) {
        zipOpenNewFileInZip(_zip, afileName, &zipInfo, NULL, 0, NULL, 0, NULL, Z_DEFLATED, Z_BEST_SPEED);
    } else {
