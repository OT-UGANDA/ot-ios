...
#define kTabLocation 1.0            // 1.0: Top, 0.0: Bottom.
#define kStartFromSecondTab 0.0     // 1.0: YES, 0.0: NO, defines if view should appear with the 1st or 2nd tab.
#define kCenterCurrentTab 0.0       // 1.0: YES, 0.0: NO, defines if tabs should be centered, with the given tabWidth.
#define kFixFormerTabsPositions 0.0 // 1.0: YES, 0.0: NO, defines if the active tab should be placed margined by the offset amount to the left. Effects only the former tabs. If set 1.0 (YES), first tab will be placed at the same position with the second one, leaving space before itself.
#define kFixLatterTabsPositions 0.0 // 1.0: YES, 0.0: NO, like ViewPagerOptionFixFormerTabsPositions, but effects the latter tabs, making them leave space after themselves.
...
- (void)drawRect:(CGRect)rect {
    ...
    // Draw left line by chuyentt
    bezierPath = [UIBezierPath bezierPath];
    [bezierPath moveToPoint:CGPointMake(0.0, CGRectGetHeight(rect)/3.0)];
    [bezierPath addLineToPoint:CGPointMake(0.0, CGRectGetHeight(rect))];
    [[UIColor otLightGrey] setStroke];
...
- (void)layoutSubviews {
    ...
    CGFloat bottomLayoutGuide = CGRectGetHeight(self.view.frame) - [self.tabHeight floatValue];
    if (IOS_VERSION_7) {
        topLayoutGuide = 20.0;
        if (self.navigationController && !self.navigationController.navigationBarHidden) {
            topLayoutGuide += self.navigationController.navigationBar.frame.size.height;
            
            // modified by chuyentt
            if (self.navigationController && ![self.navigationController.navigationBar isTranslucent]) {
                topLayoutGuide -= self.navigationController.navigationBar.frame.size.height;
            }
            if (![[UIApplication sharedApplication] isStatusBarHidden] && ![self.navigationController.navigationBar isTranslucent]) {
                ...
                topLayoutGuide -= statusBarHeight;
            }
        }
    ...
    frame.origin.y = [self.tabLocation boolValue] ? topLayoutGuide : bottomLayoutGuide;
    ...
    frame.size.width = CGRectGetWidth(self.view.frame);
    // modified by chuyentt
    frame.size.height = CGRectGetHeight(self.view.frame) - (topLayoutGuide + CGRectGetHeight(self.tabsView.frame)) - CGRectGetHeight(self.tabBarController.tabBar.frame) - CGRectGetHeight(self.navigationController.toolbar.frame);
    if ([self.navigationController.toolbar isHidden]  ) {
        frame.size.height -= CGRectGetHeight(self.navigationController.toolbar.frame);
    } else {
        frame.size.height += CGRectGetHeight(self.navigationController.toolbar.frame);
    }
